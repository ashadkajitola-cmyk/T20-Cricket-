...
<script>
    // 🔥 FIREBASE CONFIGURATION (Yahan apna Firebase details daalein)
    ...
    function sendOTP() {
        ...
        db.ref('active_sessions/' + phone).once('value', (snapshot) => {
            ...
            db.ref('device_logged_numbers/' + phone).once('value', (logSnap) => {
                tempPhone = phone;
    ...
    function verifyOTP() {
        ...
        let myDeviceId = localStorage.getItem('my_device_id');
        db.ref('active_sessions/' + tempPhone).set(myDeviceId);
    ...
    // DIRECT POINTS SEND TO USER (FIXED & FULLY WORKING)
    function sendPointsToUserAction() {
        ...
        userRef.once('value', (snapshot) => {
            ...
            userRef.set({
                ...
                if(error) {
                    alert("Points bhejne mein error aayi.");
                } else {
                    document.getElementById('adminTargetPhone').value = "";
                    document.getElementById('adminSendPoints').value = "";
                    alert(`Successfully ${pointsToAdd} points user (${targetPhone}) ke wallet mein add kar diye gaye hain! Total Points: ${updatedPoints}`);
                }
            });
        });
    }
    ...
    function buySubscription(cost, planName, extraDays, extraMins) {
        ...
        db.ref('users/' + currentPhone).once('value', (snapshot) => {
            ...
            db.ref('users/' + ADMIN_NUMBER).once('value', (adminSnap) => {
                let adminData = adminSnap.val() || { points: 50000 };
    ...
    function populateAdminGroupDropdown(tablesObj) {
        ...
        groupSelect.innerHTML = "<option value='-- Table Chunein --'>-- Table Chunein --</option>";
